<template>
      ...
      <form class="mt-8 space-y-6" @submit="login">
        <div v-if="erroMsg" class="flex items-center justify-between py-3 px-5 bg-red-500 text-white rounded">
          {{erroMsg}}
          <span @click="erroMsg = ''" class="h-8 w-8 flex items-center justify-center cursor-pointer rounded-full transition-colors hover:bg-[rgba(0,0,0,0.2)]">
...
function login(event){
  ...
  store.dispatch('login', user)
    .then(() => {
      ...
      erroMsg.value = err.response.data.error;
    })
}
</script>